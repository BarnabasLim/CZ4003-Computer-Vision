...
def dilate(image):
    ...
    img_dialate=cv2.dilate(image, kernel, iterations = 1)
    data=np.ascontiguousarray(img_dialate)
...
def erode(image):
    ...
    data=np.ascontiguousarray(img_erode)
    data=matlab.double(data.tolist())
    return data
...
def opening(image):
    ...
    data=np.ascontiguousarray(img_open)
    data=matlab.double(data.tolist())
    return data
...
#images = {'Gray': img,'Threshold': img_deskew}
#display_multiple_img(images, 1, 2)
#print(tesseractOCR(img,1))
